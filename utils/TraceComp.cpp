...
extern int SamCompare(int nchain, int burnin, int stop, string* ChainName, double& disc,
                      double& overlap, double& effsize, string outname);
...
int main(int argc, char* argv[]) {
    ...
    if (argc == 1) {
        ...
        cerr << "\t-x <burnin>. default burnin = 20% of the chain\n";
        cerr << '\n';
        cerr << "\t measure the effective sizes and overlap between 95% CI of several independent "
                "chains\n";
        cerr << '\n';
        exit(1);
    }
    ...
    for (int p = 0; p < P; p++) {
        if (!ifstream((ChainName[p] + ".trace").c_str())) {
            if (!ifstream(ChainName[p].c_str())) {
                cerr << "error : does not find file named " << ChainName[p] << " or "
                     << ChainName[p] + ".trace\n";
                exit(1);
            }
        } else {
