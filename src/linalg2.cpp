...
|	EigenRealGeneral
|
|	Calculate eigenvalues and eigenvectors of a general real matrix assuming that all
eigenvalues
...
/*  *work = work vector of size 'n'                                         */
{
    ...
    for (i = 0; i < n; i++) {
        if (vi[i] != 0.0) return RC_COMPLEX_EVAL;
    }
...
|
|		z  contains the real and imaginary parts of the eigenvectors.  If the j-th
eigenvalue is
|		real, the j-th column of  z  contains its eigenvector.  If the j-th eigenvalue is
complex
|		with positive imaginary part, the j-th and (j+1)-th columns of  z  contain the real
and
|		imaginary parts of its eigenvector.  The conjugate of this vector is the
eigenvector for
|		the conjugate eigenvalue.
|
|		ierr  is an integer output variable set equal to an error completion code described
in the
...

{
    ...
    if (ierr == 0) BalBak(n, is1, is2, fv1, n, z);
...
|	follow.
|
|	This subroutine is a translation of the algol procedure BALANCE, Num. Math. 13,
293-304(1969)
...
|	denotes the index interchanged with j during the permutation step, and that the elements of
the
...
|	follow.
|
|	This subroutine is a translation of the algol procedure ELMHES, Num. Math. 12,
349-368(1968) by
|	Martin and Wilkinson.  Handbook for Auto. Comp., Vol. II-Linear Algebra, 339-358 (1971).
|
|	Given a real general matrix, this subroutine reduces a submatrix situated in rows and
columns
|	low through high to upper Hessenberg form by stabilized elementary similarity
transformations.
|
|	ON INPUT:
|
|		n is the order of the matrix.
|
|		low and high are integers determined by the balancing subroutine BALANC.  If BALANC
has not
...
|	ON OUTPUT:
|
|		a contains the Hessenberg matrix.  The multipliers which were used in the reduction
are
        ...
        if (x != 0.0) {
            ...
            for (i = mp1; i <= high; i++) {
                ...
                if (y != 0.0) {
                    ...
                    for (j = 0; j <= high; j++) a[j][m] += y * a[j][i];
                }
            }
        }
    }
...
|	follow.
|
|	This subroutine is a translation of the algol procedure ELMTRANS,  Num. Math. 16, 181-204
(1970)
...
|		n is the order of the matrix.
|
|		low and high are integers determined by the balancing subroutine Balanc.  if
Balanc has
|		not been used, set low=1, high=n.
|
|		a contains the multipliers which were used in the reduction by ElmHes in its lower
triangle
|		below the subdiagonal.
|
|		intchg contains information on the rows and columns interchanged in the reduction
by ElmHes.
...

{
    ...
    for (j = 0; j < n; j++) {
        for (i = 0; i < n; i++) z[i][j] = 0.0;
        z[j][j] = 1.0;
    }

    for (mp = high - 1; mp >= low + 1; mp--) {
        for (i = mp + 1; i <= high; i++) z[i][mp] = a[i][mp - 1];
...
|	follow.
|
|	This subroutine is a translation of the algol procedure HQR2, Num. Math. 16, 181-204 (1970)
by
...
|	ElTran or OrtHes  and  OrTran  have been used to reduce this general matrix to Hessenberg
form
...
|
|		z contains the transformation matrix produced by ElTran after the reduction by
ElmHes, or
...
|		wr and wi contain the real and imaginary parts, respectively, of the eigenvalues.
The
|		eigenvalues are unordered except that complex conjugate pairs of values appear
consecutively
|		with the eigenvalue having the positive imaginary part first.  If an error exit is
made, the
...
eigenvalue is
|		real, the i-th column of z contains its eigenvector.  If the i-th eigenvalue is
complex with
|		positive imaginary part, the i-th and (i+1)-th columns of z contain the real and
imaginary
|		parts of its eigenvector.  The eigenvectors are unnormalized.  If an error exit is
made,
...

{
    ...
    double norm = 0, p = 0, q = 0, r = 0, s = 0, t = 0, w = 0, x = 0, y, ra, sa, vi, vr, zz = 0,
           tst1, tst2;
                    ...
                    if (wi[i] < 0.0) {
                        ...
                        if (wi[i] == 0.0)
                            ...
                            if (fabs(x) > fabs(zz) + fabs(q)) {
                                ...
                                CDiv(-r - y * h[i][na], -s - y * h[i][en], zz, q, &h[i + 1][na],
                                     &h[i + 1][en]);
                        }
...
|	follow.
|
|	This subroutine is a translation of the algol procedure BALBAK, Num. Math. 13, 293-304
(1969)
|	by Parlett and Reinsch.  Handbook for Auto. Comp., vol. II-Linear Algebra, 315-326 (1971).
|
|	This subroutine forms the eigenvectors of a real general matrix by back transforming those
of
...
|
|		scale contains information determining the permutations and scaling factors used by
Balanc.
|
|		m is the number of columns of z to be back transformed.
|
|		z contains the real and imaginary parts of the eigenvectors to be back transformed
in its
|		first m columns.
|
|	ON OUTPUT:
|
|		z contains the real and imaginary parts of the transformed eigenvectors in its
first m
|		columns.
*/
