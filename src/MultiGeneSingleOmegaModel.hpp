...
class MultiGeneSingleOmegaModel : public ProbModel	{
	...
	void RecursiveCollectLengthSuffStat(int gene, const Link* from)	{
		if (! from->isRoot())	{
			for (int i=0; i<Nsite[gene]; i++)	{
				phyloprocess[gene]->AddLengthSuffStat(i,from->Out(),genebranchlengthcount[gene][from->GetBranch()->GetIndex()],genebranchlengthbeta[gene][from->GetBranch()->GetIndex()]);
			}
		}
		for (const Link* link=from->Next(); link!=from; link=link->Next())	{
	...
	double GetSuffStatLogProb(int gene)	{
		return suffstatlogprob[gene];
	}

	void BackupSuffStatLogProb(int gene)	{
		bksuffstatlogprob[gene] = suffstatlogprob[gene];
	}

	void RestoreSuffStatLogProb(int gene)	{
		suffstatlogprob[gene] = bksuffstatlogprob[gene];
	}
