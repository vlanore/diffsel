...
void MGSRFitnessCodonUsageSubMatrix::ComputeStationary() const {
...
void MGSRFitnessCodonUsageSubMatrix::ComputeArray(int i) const {
    double total = 0;
...
void MGMSFitnessCodonUsageSubMatrix::ComputeStationary() const {
    // compute stationary probabilities
    double total = 0;
    for (int i = 0; i < GetNstate(); i++) {
        mStationary[i] = NucMatrix->Stationary(GetCodonPosition(0, i)) *
...
void MGMSFitnessCodonUsageSubMatrix::ComputeArray(int i) const {
    double total = 0;
    for (int j = 0; j < GetNstate(); j++) {
        if (i != j) {
            int pos = GetDifferingPosition(i, j);
