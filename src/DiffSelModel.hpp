...
class DiffSelModel : public ProbModel {
	...
	// auxiliary array
	// condalloc[k][l] is 1 iff condition l should recompute its fitness profiles whenever delta[k] has changed
	// bool ?
	int** condalloc;
	...
	void ReadFiles(string datafile, string treefile)	{
		...
		// nodes start at 0 (for the root), and nodes 1..Ntaxa are tip nodes (corresponding to taxa in sequence alignment)
		tree->SetIndices();
	...
	double SiteGlobalProfileLogProb(int i)	{
	...
	// for nuc rate parameters: collapsed onto the 4 nucleotide states (and summed over all sites and conditions)
	/*
