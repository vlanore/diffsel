...
class PhyloProcess	{
  ...
  public:
    ...
    PhyloProcess(Tree* intree, SequenceAlignment* indata, double* inbranchlength, double* insiterate, SubMatrix*** insubmatrix, double** inrootfreq, SubMatrix** inrootsubmatrix);
	...
	return branchlength[branch->GetIndex()];
    }
    ...
    bool isMissing(const Node *node, int site) { return false; return missingmap[node][site]; }
  ...
  protected:
    ...
    */
	...
	BranchSitePath* SamplePath(int stateup, int statedown, double time, double rate, SubMatrix* matrix);
